...
func (c *Client) GetPolicy(policyId int) (*Policy, error) {
	uri := fmt.Sprintf("%s/service/public/v2/api/policy/%d", c.BaseURL, policyId)
...
func (c *Client) DeletePolicy(policyId int) error {
	uri := fmt.Sprintf("%s/service/public/v2/api/policy/%d", c.BaseURL, policyId)
